...
def calculate_new_position_notional():
    ...
    mode = load_buy_size_mode() if "load_buy_size_mode" in globals() else ("full" if FULL_BUY_WHEN_ONE_POSITION else "partial")
    sizing_equity = effective_trading_equity(equity) if "effective_trading_equity" in globals() else equity
    if mode == "full" and int(MAX_POSITIONS) <= 1:
        usable_cash = max(0.0, buying_power - (FULL_BUY_CASH_BUFFER if "FULL_BUY_CASH_BUFFER" in globals() else 2.0))
...
# =========================
# BUY SIZE MODE + PREVIEW
# =========================
BUY_SIZE_MODE_FILE = "buy_size_mode.json"
BUY_SIZE_MODE = os.getenv("BUY_SIZE_MODE", "full").lower()
if BUY_SIZE_MODE not in ("full", "partial"):
    BUY_SIZE_MODE = "full"

def load_buy_size_mode() -> str:
    try:
        data = safe_load_json(BUY_SIZE_MODE_FILE, {"mode": BUY_SIZE_MODE}) if "safe_load_json" in globals() else {"mode": BUY_SIZE_MODE}
        mode = str(data.get("mode", BUY_SIZE_MODE)).lower().strip()
...
def save_buy_size_mode(mode: str) -> str:
    ...
    if mode not in ("full", "partial"):
        mode = "full"
    try:
        if "safe_save_json" in globals():
            safe_save_json(BUY_SIZE_MODE_FILE, {"mode": mode, "savedAt": datetime.now(UTC).isoformat()})
    except Exception as e:
...
def api_buy_size_mode(request: Request, payload: dict = Body(...)):
    ...
    return {"ok": True, "message": f"Buy size mode saved as {saved_mode}", "buySizeMode": saved_mode, "fullBuyWhenOnePosition": saved_mode == "full", "preview": buy_size_preview_payload()}
