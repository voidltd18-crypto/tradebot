...
def _usd_to_gbp_rate_safe() -> float:
    try:
        r = float(get_usd_to_gbp_rate())
        return r if r > 0 else 0.78
    except Exception:
        return 0.78

def _usd_to_gbp_safe(usd: float) -> float:
    try:
        return money_gbp(float(usd or 0))
    except Exception:
        return float(usd or 0) * _usd_to_gbp_rate_safe()

def _gbp_to_usd_safe(gbp: float) -> float:
    rate = _usd_to_gbp_rate_safe()
...
def load_trading_cap_gbp() -> float:
    ...
    if env_usd > 0:
        return _usd_to_gbp_safe(env_usd)
...
def trading_cap_usd() -> float:
    return _gbp_to_usd_safe(load_trading_cap_gbp())
...
def banking_payload():
    ...
    return {
        ...
        "accountEquityGbp": round(_usd_to_gbp_safe(equity), 2),
        "buyingPower": round(buying_power, 2),
        "buyingPowerGbp": round(_usd_to_gbp_safe(buying_power), 2),
        "effectiveTradingEquity": round(effective, 2),
        "effectiveTradingEquityGbp": round(_usd_to_gbp_safe(effective), 2),
        "bankedProfitCashBuffer": round(banked, 2),
        "bankedProfitCashBufferGbp": round(_usd_to_gbp_safe(banked), 2),
