...
def build_weekly_universe(force=False):
    ...
    chosen = [
        {
            "symbol": s,
            "score": float(100 - i * 3),
            "reason": "quality-only universe | stock-memory scoring disabled",
            "status": "active",
        }
        for i, s in enumerate(chosen_symbols)
    ]

    try:
        save_weekly_universe(chosen, "quality-only refresh")
    except Exception as e:
        print(f"QUALITY WEEKLY SAVE ERROR: {e}")

    try:
        current_universe[:] = chosen_symbols
    except Exception:
        globals()["current_universe"] = chosen_symbols[:]
    ...
    try:
        latest_status["autoUniverse"] = {
            "enabled": True,
            "mode": "quality-only",
            "size": len(chosen_symbols),
            "weekStart": week_start_str(),
            "activeSymbols": chosen_symbols,
            "rows": chosen,
            "lastRefresh": datetime.now(UTC).isoformat(),
            "candidatePoolSize": len(chosen_symbols),
            "keepWinners": True,
            "manualPickCount": 0,
        }
    except Exception:
    ...
    return {
        "ok": True,
        "message": f"Quality-only universe loaded with {len(chosen_symbols)} symbols",
