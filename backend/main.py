...
def current_open_position_value_usd() -> float:
    """Live open-position value used to enforce the trading cap as a hard ceiling."""
    total = 0.0
    try:
        for p in get_all_positions():
            try:
                total += float(p.get("value") or p.get("market_value") or 0.0)
            except Exception:
                pass
    except Exception:
        return 0.0
...
def calculate_new_position_notional():
    ...
    # Profit banking cap is a hard cap, not just a display value.
    # New buys can only use the remaining capped capacity after live open positions.
    ...
    remaining_cap = max(0.0, sizing_equity - open_value)
    remaining_slots = allowed_new_position_count()

    if remaining_slots <= 0 or remaining_cap <= MIN_ORDER_NOTIONAL:
        return 0.0

    # Full Buy means use the remaining capped buying capacity for the next position.
    # It must never use account equity above the saved trading cap.
    ...
    # Partial Buy splits only the remaining capped capacity across remaining slots.
    target_value = remaining_cap / max(1, remaining_slots)
