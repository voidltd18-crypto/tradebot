...
def latest_pdt_block_event_for_symbol(symbol: str):
    ...
    try:
        for event in reversed(alpaca_rejection_events):
            if str(event.get("symbol", "")).upper().strip() != sym:
                continue
            ...
            if "PDT" in event_type or is_likely_pdt_error(error_text) or is_likely_pdt_error(message):
                return event
    except Exception:
        return None
...
def get_all_positions():
    ...
    for pos in raw_positions:
        try:
            ...
            pdt_block_event = latest_pdt_block_event_for_symbol(symbol)
