...
const BOT_VERSION = "v15.13-decoupled-reports-frontend";
...
const [banking, setBanking] = useState<AnyObj>({});
  ...
  const displayClosedTrades = [...closedTrades].sort((a: AnyObj, b: AnyObj) => closedTradeTime(b) - closedTradeTime(a)).slice(0, 80);
  const tradeDate = (t: AnyObj) => {
    ...
    return String(t?.day || t?.date || "—");
  };
  const tradeTime = (t: AnyObj) => {
    if (t?.time) return String(t.time).slice(0, 8);
    const raw = t?.timestamp || "";
    const d = new Date(String(raw));
    if (Number.isFinite(d.getTime())) return d.toLocaleTimeString("en-GB", { hour12:false });
    return "—";
  };
...
const loadReports = useCallback(async (force = false) => {
    if (!authToken || reportsLoading) return;
    if (!force && reportsUpdatedAt && Date.now() - Date.parse(reportsUpdatedAt) < 60000) return;
    ...
    try {
      const json = await fetchJsonWithTimeout(
        `${API_URL}/reports`,
        { cache: "no-store", headers: secureHeaders },
        30000,
      );
      ...
      setReportsError(timedOut ? "Reports are taking too long. The live dashboard remains connected." : (e?.message || "Reports unavailable."));
      console.error("Reports load failed", e);
    } finally {
      setReportsLoading(false);
    }
  }, [authToken, reportsLoading, reportsUpdatedAt, secureHeaders]);
...
const fetchData = useCallback(async (force = false) => {
    ...
    try {
      const [statusRes, bankingRes] = await Promise.allSettled([
        fetchJsonWithTimeout(`${API_URL}/status`, { cache: "no-store", headers: secureHeaders }, 15000),
        fetchJsonWithTimeout(`${API_URL}/banking-status`, { cache: "no-store", headers: secureHeaders }, 15000),
      ]);
  ...
  }, [authToken, selectedSymbol, secureHeaders]);
  ...
  useEffect(() => {
    if (authToken && tab === "reports") loadReports(false);
  }, [authToken, tab, loadReports]);
  ...
  return <div className="app">
    ...
    {tab==="reports" && <main className="reports-page">
      <div className="actions report-actions"><button onClick={() => loadReports(true)} disabled={reportsLoading}>{reportsLoading ? "Loading Reports..." : "Refresh Reports"}</button><button className="danger" onClick={resetBaseline}>Reset PnL Baseline</button>
          <input
            className="input"
      ...
      {reportsLoading && <p className="notice">Reports are loading in the background. Live balances, positions and manual controls remain available.</p>}
      {reportsError && <p className="notice loss">{reportsError} <button onClick={() => loadReports(true)}>Retry Reports</button></p>}
      {!reportsLoading && !reportsError && reportsUpdatedAt && <p className="muted">Reports updated {new Date(reportsUpdatedAt).toLocaleTimeString("en-GB", { hour12:false })}.</p>}
      <section className="stats">
      ...
      <Card title="Closed Trade History">
        <div className="actions">
          <button onClick={() => action("/backfill-trades")}>Backfill Past Trades</button>
          <button onClick={() => action("/backfill-trades-limited")}>Quick Backfill</button>
          <button onClick={() => action("/rebuild-closed-trades")}>Rebuild Closed Trades</button>
        </div>
        <div className="table-wrap"><table><thead><tr><th>Date</th><th>Time</th><th>Symbol</th><th>Entry</th><th>Exit</th><th>Qty</th><th>PnL</th><th>%</th></tr></thead><tbody>{displayClosedTrades.map((t:AnyObj,i:number)=><tr key={i}><td>{tradeDate(t)}</td><td>{tradeTime(t)}</td><td>{t.symbol}</td><td>{usd(t.entryPrice)}</td><td>{usd(t.exitPrice)}</td><td>{Number(t.qty || 0).toFixed(4)}</td><td className={tone(t.pnl)}>{gbp(Number(t.pnlGbp ?? Number(t.pnl || 0) * rate))} / {usd(t.pnl)}</td><td className={tone(t.pnl)}>{pct(t.pnlPct)}</td></tr>)}{!displayClosedTrades.length && <tr><td colSpan={8}>No matched closed trades yet.</td></tr>}</tbody></table></div>
      </Card>
    </main>}
