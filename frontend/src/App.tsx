...
const fetchData = useCallback(async (force = false) => {
  ...
  async function reportAction(endpoint:string, label:string) {
    if (!token) {
      setMessage("Please login first.");
      return;
    }
    setMessage(`${label} started... please wait.`);
    try {
      ...
      setMessage(e?.message || `${label} failed.`);
    }
  }

  function tradeDateTime(t:AnyObj) {
    const raw = t?.timestamp || t?.datetime || t?.dateTime || t?.closedAt || t?.time;
    if (raw) {
  ...
  return <div className="app">
    ...
    {tab==="reports" && <main className="reports-page">
      <div className="actions report-actions"><button onClick={() => fetchData(true)}>Refresh Reports</button><button onClick={() => reportAction("/backfill-trades", "Backfill Past Trades")}>Backfill Past Trades</button><button className="ghost" onClick={() => reportAction("/backfill-trades-limited", "Quick Backfill")}>Quick Backfill</button><button onClick={() => reportAction("/rebuild-closed-trades", "Rebuild Closed Trades")}>Rebuild Closed Trades</button><button className="danger" onClick={resetBaseline}>Reset PnL Baseline</button>
          <input
            className="input"
