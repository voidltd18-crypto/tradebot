...
const [banking, setBanking] = useState<AnyObj>({});
  ...
  const [targetEquityInput, setTargetEquityInput] = useState<string>(() => localStorage.getItem("tradebot_target_equity_gbp") || "");
  ...
  const equityGbpLive = Number(data?.account?.equity || 0) * rate;
  const targetEquityGbp = Number(targetEquityInput || 0);
  const targetEquityReached = targetEquityGbp > 0 && equityGbpLive >= targetEquityGbp;
...
const fetchData = useCallback(async (force = false) => {
  ...
  return <div className="app">
    ...
    <section className="stats">
      ...
      <section
        className="card stat"
        style={targetEquityReached ? {
          borderColor: "rgba(34, 197, 94, 0.85)",
          boxShadow: "0 0 24px rgba(34, 197, 94, 0.35), inset 0 0 16px rgba(34, 197, 94, 0.08)",
          background: "linear-gradient(135deg, rgba(34,197,94,0.12), rgba(15,23,42,0.96) 60%)"
        } : undefined}
      >
        <span>Price Goal</span>
        <input
          value={targetEquityInput}
          onChange={(e) => {
            const value = e.target.value;
            setTargetEquityInput(value);
            localStorage.setItem("tradebot_target_equity_gbp", value);
          }}
          placeholder="Target £"
          inputMode="decimal"
          style={{
        ...
        />
        <strong className={targetEquityReached ? "gain" : ""}>
          {targetEquityReached ? "TARGET REACHED ✅" : targetEquityGbp > 0 ? gbp(targetEquityGbp) : "Set target"}
        </strong>
        <small>Current {gbp(equityGbpLive)}</small>
      </section>
    </section>
